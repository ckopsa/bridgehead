...
//! | `stance` | exempt | The row above, said in one word. A stance installs exactly what those verbs install, so charging it a link would price as micro the single sentence that is *most* clearly doctrine — and would make the preset strictly worse than typing its five parts out, which is C4 inverted. |
